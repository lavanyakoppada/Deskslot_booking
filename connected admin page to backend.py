...
class SelectBlock:
    ...
    def bookdesks_in_digilib(self):
        ...
        select = SelectDeskLib(root, self.studId, start_time_str, end_time_str)
    ...
    def bookdesks_in_d211(self):
        ...
        conn.commit()
        self.time_frame.destroy()
        select = SelectDeskDBlock(root, self.studId, start_time_str, end_time_str)

class SelectDeskDBlock:
    def __init__(self, root, studId, start, end) -> None:
        self.root = root
        self.studId = studId
    ...
    def create_seat_map(self):
        ...
        for i in range(7):
            for j in range(10):
                ...
                self.query2 = f"SELECT CompId FROM DBlockVacancy WHERE CompId LIKE '{self.s}'"
                cursor.execute(self.query2)
