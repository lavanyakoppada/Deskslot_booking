...
class SelectBlock:
    ...
    def bookdesks_in_digilib(self):
        ...
        self.time_frame.destroy()
        query = f"DELETE FROM LibVacancy WHERE EndTime LIKE {current_time_str}"
        cursor.execute(query)
        conn.commit()
        query = f"DELETE FROM Booking WHERE EndTime LIKE '{current_time_str}'"
        cursor.execute(query)
        conn.commit()
    ...
    def bookdesks_in_d211(self):
        ...
        current_time_str = convertToTime(f"{datetime.now().hour}:00")
        self.time_frame.destroy()
        query = f"DELETE FROM DBlockVacancy WHERE EndTime LIKE '{current_time_str}'"
        cursor.execute(query)
        conn.commit()
        query = f"DELETE FROM Booking WHERE EndTime LIKE '{current_time_str}'"
        cursor.execute(query)
...
class SelectDeskDBlock:
    def __init__(self, root, studId, date, start, end) -> None:
        ...
        self.login_btn = Button(self.root, text = 'SUBMIT', bg = 'white', fg = 'steel blue', command = self.submit, font = fonts, cursor = 'hand2', activebackground = 'Rosy Brown1')
        self.login_btn.place(x = 390, y = 350)
...
class confirmation:
    def __init__(self, root, studId):
        ...
        self.time_label_text = Label(self.confirm_frame, text = s, font = fonts, bg = 'dark turquoise', fg = 'navy', width = 10)
        self.time_label_text.place(x = 210, y = 675)
...
class D_blockRepair:
    ...
    def create_seat_map(self):
        ...
        for i in range(7):
            for j in range(10):
                seat_label = tk.Label(
                    self.root, text=f"D-{self.seat_count+1}", width=8, height=2, relief="solid"
                )
                seat_label.grid(row=i, column=j, padx=5, pady=5)
                seat_label.bind("<Button-1>", lambda event, row=i, col=j: self.on_seat_click(row, col))
                self.seat_count += 1

        self.login_btn = Button(self.root, text = 'UPDATE', bg = 'white', fg = 'steel blue', command = self.insert, font = fonts, cursor = 'hand2', activebackground = 'Rosy Brown1')
        self.login_btn.place(x = 390, y = 350)
...
class Lib_Repair:
    ...
    def on_seat_click(self, row, col):
        ...
        if current_status == "available":
            self.seat_status[row][col] = "repair"
            self.selected.append(f"L-{row}{col}")
            self.update_seat_color(row, col, "red")
        if current_status == "repair":
            self.seat_status[row][col] = "available"
            self.update_seat_color(row, col, "white")
            self.selected.remove(f"L-{row}{col}")
